...
export default function RoomLayout({  }: RoomLayout) {
...
useEffect(() => {
 ...
 useEffect(() => {
  const fetchRooms = async () => {
    try {
      const res = await axios.get<ApiResponse>("https://nascent.virtualshopbd.com/api/bouquet");
...
const handleBookNow = (type: string) => {
  ...
  const handleSearchsdata = () => {
    ...
    router.push(`/selectbouquet?arrival=${a}&departure=${d}&guests=${guests}&roomType=${roomType}`);
  };
