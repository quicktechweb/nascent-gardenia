...
export default function RoomsTable() {
  ...
  const handleDeleteRoom = async (roomId: string) => {
    ...
    try {
      await axios.delete(`https://nascent.virtualshopbd.com/api/rooms/${roomId}`);
 ...
 const handleEditSave = async () => {
  ...
  try {
    await axios.put(`https://nascent.virtualshopbd.com/api/rooms/updateRootAndFirst/${editRoom.parentId}`, {
  ...
  <table className="min-w-full table-fixed text-sm text-gray-800">
    ...
    {/* ===== TABLE HEADER ===== */}
    <thead className="bg-gradient-to-r from-[#c78436] to-[#c78436] text-white sticky top-0 z-10">
      <tr>
        {[
          " Image",
          "Title",
          ...
          <th
            key={i}
            className="px-4 py-3 text-left font-semibold uppercase tracking-wide border-r last:border-r-0"
          >
            {h}
          </th>
        ))}
      ...
      {rooms?.map((room, roomIndex) => {
        ...
        return (
          ...
          >
            ...
            {/* ===== ROOM DATA ===== */}
            <td className="px-3 py-3 font-semibold truncate max-w-[160px]" title={first.title}>
              {first.title}
            </td>

            <td className="px-3 py-3 truncate max-w-[120px]" title={first.roomType}>
              {first.roomType}
            </td>
              ...
              <div className="grid grid-cols-2 gap-2">
                {first.carouselImages.map((img, i) => (
                  <img
                    key={i}
                    src={img}
                    className="h-10 w-full object-cover rounded shadow-sm"
                  />
                ))}
              </div>
            </td>
            ...
            <td className="px-3 py-3">
              <button
                className="bg-[#c78436] hover:bg-yellow-500 text-white px-4 py-1.5 rounded-md text-sm font-semibold shadow transition"
                onClick={() =>
                  setEditRoom({
                    ...first,
