...
export default function RoomsTable() {
  ...
  const fetchRooms = async () => {
    try {
      const res = await axios.get("https://nascent.virtualshopbd.com/api/fitness");
  ...
  const handleDeleteRoom = async (roomId: string) => {
    ...
    try {
      await axios.delete(`https://nascent.virtualshopbd.com/api/fitness/${roomId}`);
...
const handleEditSave = async () => {
  ...
  try {
    await axios.put(
      `https://nascent.virtualshopbd.com/api/fitness/updateRootAndFirst/${editRoom.parentId}`,
      {
...
type MealKey = "morning" | "evening" | "night";
  ...
  return (
    <div className="p-8 bg-white text-black min-h-screen">
      <h2 className="text-3xl font-bold mb-6">Show Fitness All Data</h2>
          ...
          <tr key={first._id} className="hover:bg-gray-50 transition duration-200">
            ...
            {roomIndex === 0 ? (
              <>
                ...
                <td className="px-3 py-3">{room.roomshortdescription}</td>
            ...
            {/* ===== CHILD ROOM DATA ===== */}
            <td className="px-3 py-3 font-medium">{first.morning}</td>
            <td className="px-3 py-3">{first.evening}</td>
            <td className="px-3 py-3">{first.night}</td>
...
{(["morning", "evening", "night"] as MealKey[]).map((key) => (
