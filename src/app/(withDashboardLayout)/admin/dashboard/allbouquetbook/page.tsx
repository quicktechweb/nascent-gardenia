...
export default function BouquetPendingPage() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await fetch("https://nascent.virtualshopbd.com/api/bouquetbooking/all");
 ...
 const filteredBookings = bookings.filter((b) => {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const res = await fetch(`https://nascent.virtualshopbd.com/api/bouquetbooking/update-status/${id}`, {
        method: "PATCH",
...
const paginatedBookings = filteredBookings.slice(
  ...
  <span className="text-sm font-medium">entries</span>
      ...
      <div className="flex justify-between items-center mb-4">
        <p className="font-medium">Total Booking: {bookings.length}</p>
        <input
      type="text"
