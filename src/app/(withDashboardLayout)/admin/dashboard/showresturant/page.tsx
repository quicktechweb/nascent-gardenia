...
export default function RoomsTable() {
  ...
  const fetchRooms = async () => {
    try {
      const res = await axios.get("https://nascent.virtualshopbd.com/api/restaurants");
  ...
  const handleDeleteRoom = async (roomId: string) => {
    ...
    try {
      await axios.delete(`https://nascent.virtualshopbd.com/api/restaurants/${roomId}`);
...
const handleEditSave = async () => {
  ...
  try {
    await axios.put(
      `https://nascent.virtualshopbd.com/api/restaurants/updateRootAndFirst/${editRoom.parentId}`,
      {
...
type MealKey = "breakfast" | "lunch" | "dinner";
  ...
  return (
    <div className="p-8 bg-white text-black min-h-screen">
      <h2 className="text-3xl font-bold mb-6">Show Resturant All Data</h2>
  ...
  <table className="min-w-full table-auto text-sm text-gray-700">
    ...
    <thead className="bg-gradient-to-r from-[#c78436] to-[#a6712e] text-white sticky top-0 z-10">
      <tr>
        {[
          ...
          <th
            key={i}
            className="px-4 py-3 text-left font-semibold uppercase tracking-wide border-r last:border-r-0 select-none"
          >
            {h}
          </th>
        ))}
    ...
    <tbody className="divide-y divide-gray-200">
      {rooms?.map((room, roomIndex) => {
        const first = room.roomsdata[0]; // always first child room

        return (
          <tr
            key={first._id}
            className="hover:bg-yellow-50 transition duration-200 cursor-pointer"
          >
            {/* PARENT DATA */}
            {roomIndex === 0 ? (
              <>
                ...
                <td className="px-3 py-3 font-medium truncate max-w-[160px]" title={room.herotext}>
                  {room.herotext}
                </td>
                <td className="px-3 py-3 truncate max-w-[140px]" title={room.roomshorttile}>
                  {room.roomshorttile}
                </td>
                <td className="px-3 py-3 truncate max-w-[220px]" title={room.roomshortdescription}>
                  {room.roomshortdescription}
                </td>
                <td className="px-3 py-3 truncate max-w-[160px]" title={room.roomheader}>
                  {room.roomheader}
                </td>
                <td className="px-3 py-3 truncate max-w-[220px]" title={room.herodescription}>
            ...
            {/* CHILD ROOM DATA */}
            <td className="px-3 py-3 font-semibold">{first.breakfast}</td>
            <td className="px-3 py-3 font-semibold">{first.lunch}</td>
            <td className="px-3 py-3 font-semibold">{first.dinner}</td>
              ...
              <div className="grid grid-cols-2 gap-2">
                {first.carouselImages.map((img, i) => (
                  <img
                    ...
                    src={img}
                    alt={`carousel-${i}`}
                    className="w-full h-10 object-cover rounded shadow-sm border"
                  />
                ))}
              </div>
            </td>
...
{(["breakfast", "lunch", "dinner"] as MealKey[]).map((key) => (
  <div key={key} className="col-span-2">
    ...
    <input
      ...
      value={editRoom[key] || ""}
      onChange={(e) =>
        setEditRoom({ ...editRoom, [key]: e.target.value })
      }
      className="input-white"
    />
  </div>
     ...
     {/* {editRoom.availableDates && (
          <div className="col-span-2">
            <h3 className="text-xl font-bold mb-2">Available Dates</h3>
            {editRoom.availableDates.map((d, index) => (
              <div key={d._id || index} className="flex gap-4 mb-2">
                <input
                  type="date"
                  value={d.start || ""}
                  onChange={(e) => {
